...
[role="lead"]
如果您是 VMWare 用户，您可以使用 OVA 在您的 VCenter 中部署 Console 代理。OVA 下载或 URL 可通过 NetApp Console 获得。
...
NetApp 支持在 VCenter 环境中安装 Console 代理。OVA 文件包含一个预配置的虚拟机映像，您可以在 VMware 环境中部署该映像。文件下载或 URL 部署可直接从 NetApp Console 获得。它包括 Console 代理软件和自签名证书。
