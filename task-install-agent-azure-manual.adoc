...
* 安装python3
* 安装相应的 podman-compose 包
* 将 podman-compose 添加到 PATH 环境变量
* 请确认您的 Podman 版本使用的是 Netavark Aardvark DNS 而不是 CNI。
...
. 安装 Podman。
+
从 Red Hat Enterprise Linux 软件仓库获取 Podman。
...
+
请按照以下步骤安装受支持的 Docker Engine 版本。请勿安装最新版本，因为主机不支持该版本。
...
NOTE: 如果要配置透明代理，可以在安装完成后进行配置。 link:reference-agent-maint-console.html["了解代理维护控制台"]
