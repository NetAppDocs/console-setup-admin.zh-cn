...
如果 Console 代理位于 AWS Government 区域：\https://*.blob.core.windows.net \https://cloudmanagerinfraprod.azurecr.io
 a| 
在 AWS 政府区域中安装 Console 代理时，获取用于 Console 代理升级的映像。
...
在 Azure Government 区域中安装 Console 代理时，获取用于 Console 代理升级的映像。
 ...
 a| 
如果 Console 代理位于商业区域或主权区域，可以在两组端点之间进行选择：
...
+
** NetApp支持网站（如果您还没有访问控制台的权限，则需要此网站） https://mysupport.netapp.com/site/products/all/details/netapp-console/downloads-tab["NetApp 支持站点"^] ，
...
NOTE: 如果要配置透明代理，可以在安装完成后进行配置。 link:reference-agent-maint-console-proxy.html["了解代理维护控制台"]
