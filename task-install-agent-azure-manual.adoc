...
---
= 在 Azure 中手动安装 NetApp Console 代理
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
在 Azure 中部署代理将启动运行 Linux 的虚拟机，并在您选择的 VNet 中启动 Console 代理软件。


NOTE: 在与其管理的 Cloud Volumes ONTAP 系统相同的 Azure 区域或在 https://docs.microsoft.com/en-us/azure/availability-zones/cross-region-replication-azure#azure-cross-region-replication-pairings-for-all-geographies["Azure 区域对"^]中部署 Console 代理。这可确保在 Cloud Volumes ONTAP 及其关联的存储帐户之间使用 Azure 专用链接连接。

.开始之前
* link:concept-agent-install-options.html#agent-cloud-manual-deploy["了解何时在 Azure 中手动部署"]。
