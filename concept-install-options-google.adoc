...
* link:task-install-connector-google-bluexp-gcloud.html["直接从控制台创建控制台代理"]（这是标准选项）
+
此操作将在您选择的 VPC 中启动运行 Linux 和控制台代理软件的 VM 实例。

* link:task-install-connector-google-bluexp-gcloud.html["使用 Google Platform 创建控制台代理"]
+
