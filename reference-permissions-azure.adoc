...
summary: 当NetApp控制台在 Azure 中启动控制台代理时，它会将一个自定义角色附加到 VM，该 VM 为代理提供管理该 Azure 订阅中的资源和流程的权限。代理使用权限对多个 Azure 服务进行 API 调用。 
---
= 控制台代理的 Azure 权限
:hardbreaks:
:allow-uri-read: 
...
+
** link:task-install-connector-azure-marketplace.html#step-3-set-up-permissions["设置 Azure 市场部署的权限"]
...
| Microsoft.Compute/virtualMachines/删除 | 是 | 是 | 是 
