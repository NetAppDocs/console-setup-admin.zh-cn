...
---
= 安装用于 NetApp Console 访问的 CA 签名证书
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
在受限模式或专用模式下使用 NetApp Console 时，可以从部署在云区域或本地的 Console 代理虚拟机访问用户界面。默认情况下，Console 使用自签名 SSL 证书为 Console 代理上运行的基于 Web 的控制台提供安全的 HTTPS 访问。
