...
[role="lead"]
添加和管理 AWS 凭据，以便您可以从 NetApp Console 部署和管理 AWS 账户中的云资源。如果您管理多个 AWS Marketplace 订阅，则可以从凭据页面将每个订阅分配给不同的 AWS 凭据。
...
link:concept-accounts-aws.html["了解NetApp Console如何使用 AWS 凭证和权限"]。
...
NOTE: 如果您从控制台部署了控制台代理，它会自动为您部署控制台代理的账户添加 AWS 凭证。这确保了管理资源所需的必要权限。
...
如果要为 IAM 用户提供具有 AWS 密钥的 Console，则需要向该用户授予所需的权限。Console IAM 策略定义允许 Console 使用的 AWS 操作和资源。
...
设置 IAM 角色，使 NetApp Console 软件即服务 (SaaS) 应用程序能够承担该角色。
...
----
+
有关 IAM 中跨帐户资源访问的更多信息，请参见 link:https://docs.aws.amazon.com/IAM/latest/UserGuide/access_policies-cross-account-resource-access.html["AWS Identity and Access Management (IAM) 文档"^]。
...
.结果
IAM 角色现在具有所需的权限。<<add-the-credentials,您现在可以将其添加到控制台>>。
...
.开始之前
如果您刚刚创建了 IAM 角色，则可能需要几分钟才能使用它。请稍等几分钟，然后将凭据添加到 Console。
...
. 在*组织凭据*页面上，选择*添加凭据*并按照向导中的步骤进行操作。
+
.. *凭证位置*：选择*Amazon Web Services > 控制台*。
...
有关详细信息，请参阅  https://docs.netapp.com/us-en/storage-management-fsx-ontap/requirements/task-setting-up-permissions-fsx.html["Amazon FSx for ONTAP 控制台文档"^]
...
.开始之前
您需要先创建 Console 代理，然后才能配置订阅。link:concept-agent-install-options.html["了解如何创建控制台代理"].
...
. 要将凭据与现有订阅相关联，请从下拉列表中选择该订阅，然后选择 *Configure* 。
...
从 AWS Marketplace 订阅时，流程中的最后一步是将订阅与您的组织关联。如果您未完成此步骤，则无法将订阅用于您的组织。
...
.步骤
. 选择“*管理 > 凭证*”。
. 在*组织凭证*页面上，选择一组凭证的操作菜单，然后选择*编辑凭证*。
. 进行所需的更改，然后选择*应用*。
