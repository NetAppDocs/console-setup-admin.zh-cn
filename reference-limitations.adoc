...
这些限制是NetApp Console和管理设置特有的：代理、软件即服务 (SaaS) 平台等等。
...
=== 仅 Cloud Volumes ONTAP 支持 SSL 解密

仅支持与 Cloud Volumes ONTAP 关联的代理使用透明代理服务器。如果代理与其他 NetApp 数据服务一起使用，则不能对其使用透明代理。如果您将 NetApp 数据服务与 Cloud Volumes ONTAP 一起使用，请为 Cloud Volumes ONTAP 创建专用代理，您可以在其中使用透明代理服务器。
