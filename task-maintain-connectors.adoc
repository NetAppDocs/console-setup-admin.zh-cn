...
TIP: 如果您有现成的代理，您应该注意link:reference-connector-operating-system-changes.html["受支持的 Linux 操作系统的变更"]。
...
link:reference-connector-default-config.html["了解代理的默认配置"] 。
...
link:concept-connectors.html#connectors-must-be-operational-at-all-times["请注意，控制台代理必须始终处于运行状态"] 。
