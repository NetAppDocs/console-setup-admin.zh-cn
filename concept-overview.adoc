...
summary: 该控制台通过集成数据服务统一混合多云的存储管理和保护，以保护和优化数据。 
---
= 了解NetApp Console
:hardbreaks:
:allow-uri-read: 
...
您可以通过注册 SaaS 服务或将其部署在您的主权云中来开始使用NetApp Console 。在主权云中部署NetApp Console时， NetApp会限制出站连接，以满足您组织的安全性和合规性要求。当控制台部署在主权云中时，并非所有功能和服务都可用。

NetApp继续为不需要出站连接的站点提供BlueXP 。BlueXP可以安装在您的网络上，无需任何出站连接。 link:task-quick-start-private-mode.html["了解没有互联网连接的站点的BlueXP （私人模式）。"]
