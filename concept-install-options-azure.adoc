...
summary: 有几种不同的方法可以在 Azure 中创建控制台代理。直接从NetApp控制台是最常见的方式。 
---
= Azure 中的控制台代理安装选项
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
有几种不同的方法可以在 Azure 中创建控制台代理。直接从NetApp控制台是最常见的方式。

有以下安装选项可用：

* link:task-install-connector-azure-bluexp.html["直接从NetApp控制台创建控制台代理"]（这是标准选项）
+
此操作将在您选择的 VNet 中启动运行 Linux 和控制台代理软件的 VM。

* link:task-install-connector-azure-marketplace.html["从 Azure 市场创建控制台代理"]
+
