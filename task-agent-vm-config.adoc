...
summary: 您可以在部署 Console 代理后对现有 vCenter 或 ESXi 主机进行更改。例如，您可以增加托管 Console 代理的 VM 实例的 CPU 或 RAM。 
---
= 维护 vCenter 或 ESXi 主机以用于 NetApp Console 代理
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
您可以在部署 Console 代理后对现有 vCenter 或 ESXi 主机进行更改。例如，您可以增加托管 Console 代理的 VM 实例的 CPU 或 RAM。
...
编辑您的 vCenter 或 ESXi 主机中的 VM 实例设置，然后使用维护控制台应用更改。
...
查看 vSphere 客户端中代理虚拟机的网络设置，以确认或排除网络问题。您只能查看（不更新）以下网络设置：IP 地址和 DNS 详细信息。
