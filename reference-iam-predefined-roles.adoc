...
| link:reference-iam-platform-roles.html#organization-admin-roles["组织管理员"] | 允许用户无限制地访问组织内的所有项目和文件夹。可以将成员添加到任何项目或文件夹，以及执行任何任务和使用任何不具有与其关联的显式角色的数据服务。具有此角色的用户通过创建文件夹和项目、分配角色、添加用户以及管理具有正确凭据的系统来管理您的组织。这是唯一可以创建 Console 代理的访问角色。 
...
| link:reference-iam-platform-roles.html#super-admin-roles["超级观众"] | 为用户提供查看者角色的子集。此角色专为不需要跨多个用户分配 Console 职责的小型组织设计。 
|===
...
| link:reference-iam-gcnv-roles.html["Google Cloud NetApp Volumes查看器"] | 具有Google Cloud NetApp Volumes用户角色的用户可以查看Google Cloud NetApp Volumes。 
...
| link:reference-iam-backup-rec-roles.html["备份和恢复恢复管理员"] | 在 Backup and Recovery 中还原工作负载。 


| link:reference-iam-backup-rec-roles.html["备份和恢复克隆管理员"] | 在 Backup and Recovery 中克隆应用程序和数据。 
...
| link:reference-iam-data-classification.html["分类查看器"] | 允许用户查看 NetApp Data Classification 扫描结果。具有此角色的用户可以查看合规性信息并为其有权访问的资源生成报告。这些用户无法启用或禁用卷、存储桶或数据库架构的扫描。 
