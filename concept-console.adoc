...
summary: NetApp Console提供 SaaS 体验，可访问所有功能；也提供自托管选项，适用于网络连接受限的组织。 
---
= 了解NetApp Console
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
NetApp Console提供 SaaS 体验，可访问所有功能；也提供自托管选项，适用于网络连接受限的组织。

您可以通过两种方式使用NetApp Console ：作为完全托管的 SaaS 体验，或作为您自己的云环境中的自托管解决方案。使用 SaaS 选项，您可以通过 console.netapp.com 访问控制台，而NetApp管理所有基础架构、身份验证和更新。您可以通过网页浏览器直接访问所有功能。如果选择自托管选项，则需要在 AWS、Azure 或 Google Cloud 环境中安装控制台代理。该接口在本地运行，代理程序与NetApp保持有限的出站连接，以执行身份验证和更新等基本任务。

如果您的组织拥有标准的互联网连接，请选择 SaaS 体验。您可以享受最简单的设置、自动更新以及对所有NetApp存储系统、数据服务和高级功能的完全访问权限。您可以直接从控制台添加代理， NetApp会处理所有维护工作。

如果您的组织需要严格的连接性、安全性或合规性控制，例如政府或受监管行业，请选择自托管选项。您可以获得更多控制权和网络隔离，但只能管理选定的NetApp存储系统并使用有限的数据服务，并且某些功能不可用。您可以手动安装控制台代理，并直接从本地代理界面访问控制台。通过云市场或手动安装安装代理，代理仅与NetApp进行出站通信，确保没有入站互联网连接。
...
image:diagram-standard-mode.png["一张概念图，展示了标准模式的架构，其中公共互联网提供了 SaaS 层和 Auth0 身份验证。"]
...
image:diagram-restricted-mode.png["受限模式的概念图。"]
...
| 备份和恢复 | 在政府区域和商业区域受限制模式支持。不支持在具有限制模式的主权区域使用。在受限模式下， NetApp Backup and Recovery仅支持ONTAP卷数据的备份和恢复。 https://docs.netapp.com/us-en/data-services-backup-recovery/prev-ontap-protect-journey.html#support-for-sites-with-limited-internet-connectivity["查看ONTAP数据支持的备份目标列表"^]不支持应用程序数据和虚拟机数据的备份和恢复。 
