...
summary: 您可以使用NetApp Console管理 AWS 凭证和市场订阅，通过在控制台代理部署期间提供适当的 IAM 凭证，并将其与 AWS Marketplace 订阅关联，从而安全、经济高效地部署Cloud Volumes ONTAP和其他数据服务，以便在单个或多个 AWS 账户中进行计费。 
---
...
从 Console 部署 Console 代理时，需要提供 IAM 角色的 ARN 或 IAM 用户的访问密钥。身份验证方法必须具有在 AWS 中部署 Console 代理的权限。所需的权限列在link:task-install-agent-aws-console.html#aws-permissions-agent["AWS代理部署策略"]中。

当 Console 在 AWS 中启动 Console 代理时，它会为代理创建 IAM 角色和配置文件。它还附加了一个策略，该策略为 Console 代理提供管理该 AWS 帐户内的资源和流程的权限。link:reference-permissions-aws.html["查看代理如何使用权限"]。

image:diagram_permissions_initial_aws.png["显示在 AWS 账户中部署控制台代理的控制台的概念图。"]
...
image:diagram_permissions_multiple_aws.png["一张概念图，展示了另外两个账户。每个用户都有身份和访问管理 (IAM) 策略。一个与 IAM 用户关联，另一个与 IAM 角色关联。"]
...
您必须将添加到控制台代理的凭证与 AWS Marketplace 订阅关联起来，才能按小时费率 (PAYGO) 支付Cloud Volumes ONTAP费用，并通过年度合同支付其他NetApp数据服务费用。link:task-adding-aws-accounts.html#subscribe["了解如何关联 AWS 订阅"]。
...
以上各节介绍了 Console 代理的建议部署方法，该方法来自 Console。您还可以从 AWS Marketplace 在 AWS 中部署代理，也可以在您自己的 Linux 主机或 vCenter 中手动安装 Console 代理软件。
