...
summary: 了解 NetApp Console 管理的新功能。 
---
= NetApp Console 新增功能
:hardbreaks:
:allow-uri-read: 
...
.更新了本地 UI 访问的 SSL 证书
在受限模式或私有模式下使用 BlueXP 时，用户界面可从部署在您的云区域或本地的 Connector 虚拟机访问。默认情况下，BlueXP 使用自签名 SSL 证书从本地 Web 浏览器提供安全的 HTTPS 访问。
...
我们更新了 Connector 的 AWS 和 Azure 策略，以删除不再需要的权限。这些权限与 Azure 和 AWS 中的 BlueXP 边缘缓存以及 Kubernetes 集群的发现和管理相关。
...
----
这些权限是 Cloud Volumes ONTAP 支持虚拟机规模集所必需的。如果已有 Connector，并且要使用此新功能，则需要将这些权限添加到与 Azure 凭据关联的自定义策略中。
...
如果您现有的代理在RHEL 7或CentOS 7上运行，则NetApp不支持将操作系统升级或转换为RHEL 8或9。您需要在受支持的操作系统上创建新的代理。请按照以下高级步骤进行切换：
...
** https://docs.netapp.com/us-en/storage-management-fsx-ontap/use/task-create-fsx-system.html["创建或发现 FSx for ONTAP系统"^]
