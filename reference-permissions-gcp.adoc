...
---
= NetApp Console 代理的 Google Cloud 权限
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
| 计算磁盘创建 .8+| 在 Cloud Volumes ONTAP 系统上创建和管理磁盘。 | 是 | 是 | 否 
...
| 日志记录条目创建 .2+| 创建和路由用于监控、调试和审核的日志条目。 | 是 | 是 | 否 
...
添加 `compute.disks.update`权限是为了更新 Cloud Volumes ONTAP 系统中 Hyperdisk Balanced 聚合或磁盘上的 IOPS 和吞吐量参数。
