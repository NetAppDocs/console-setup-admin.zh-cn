...
| *compute.forwardingRules.create* | 创建转发规则，将流量路由到后端服务。 | 否 | 是 | 否 


| *compute.forwardingRules.delete* | 删除现有转发规则。 | 否 | 是 | 否 


| *compute.forwardingRules.get* | 获取现有转发规则的详细信息。 | 否 | 是 | 否 


| *compute.forwardingRules.setLabels* | 设置或更新组织转发规则的标签。 | 否 | 是 | 否 


| compute.globalOperations.get | 获取操作状态。 | 是 | 是 | 否 


| *compute.healthChecks.create* .4+| 创建和管理健康检查，以监控后端服务的运行状况。 | 否 | 是 | 否 
...
| *计算实例使用* | 使用虚拟机实例（启动、停止、连接操作）。 | 否 | 是 | 否 
...
| *compute.resourcePolicies.create* .3+| 创建和管理资源策略，实现资源自动化管理。 | 否 | 是 | 否 


| *compute.resourcePolicies.delete* | 否 | 是 | 否 
...
| *logging.logEntries.create* .2+| 创建并路由日志条目，用于监控、调试和审计。 | 是 | 是 | 否 


| *logging.logEntries.route* | 是 | 是 | 否 
...
| *compute.addresses.createInternal* | 在VPC网络内创建内部IP地址以进行资源分配。 | 否 | 是 | 否 


| *compute.addresses.deleteInternal* | 删除内部 IP 地址以进行资源清理。 | 否 | 是 | 否 


| *compute.addresses.setLabels* | 更新地址资源上的标签。 | 否 | 是 | 否 


| *compute.addresses.useInternal* | 网络通信请使用内部IP地址。 | 否 | 是 | 否 
