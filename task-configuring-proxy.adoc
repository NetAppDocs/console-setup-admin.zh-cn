...
IMPORTANT: 配置代理后，无法更改代理类型。如果需要更改代理类型，则必须删除 Console 代理并添加具有新代理类型的新代理。
...
您可以使用以下方法更新控制台代理现有的透明代理服务器： `proxy update`通过使用命令来移除透明代理服务器 `proxy remove`命令。更多信息，请查阅相关文档。link:reference-agent-maint-console-proxy.html["代理维护控制台"] 。
...
如果将 Console 代理配置为使用代理服务器，则可以在 Console 代理上启用直接 API 流量，以便将 API 调用直接发送到云提供商服务，而无需通过代理。在 AWS、Azure 或 Google Cloud 中运行的代理支持此选项。
