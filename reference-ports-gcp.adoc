...
summary: 该代理的 Google Cloud 防火墙规则需要同时设置入站和出站规则。NetApp Console 在您从 Console 创建 Console 代理时会自动创建此安全组。对于其他安装选项，您需要手动设置此安全组。 
---
= Google Cloud 中的 NetApp Console 代理防火墙规则
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
该代理的 Google Cloud 防火墙规则需要同时设置入站和出站规则。NetApp Console 在您从 Console 创建 Console 代理时会自动创建此安全组。对于其他安装选项，您需要手动设置此安全组。
...
| DNS | UDP | 53 | DNS | 用于 Data Classification 的 DNS 解析 
|===