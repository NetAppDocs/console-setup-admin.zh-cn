...
在标准模式下，您可以访问由NetApp作为软件即服务 (SaaS) 产品托管的基于 Web 的控制台。在开始之前，请确保您了解link:concept-modes.html["部署模式"]和link:concept-connectors.html["控制台代理"]。
...
[role="quick-margin-para"]
此时，您已登录并可以开始管理存储和使用Digital Advisor、 Amazon FSx for ONTAP、 Azure NetApp Files等服务。link:concept-connectors.html["了解没有控制台代理您可以做什么"] 。
...
* link:concept-connectors.html["详细了解何时需要控制台代理以及它们如何工作"]
...
* link:task-install-connector-on-prem.html["了解如何在本地创建控制台代理"]
