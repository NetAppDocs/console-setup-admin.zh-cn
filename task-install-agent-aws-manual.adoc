...
---
= 在 AWS 中为 NetApp Console 手动部署 Console 代理
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
.开始之前
* link:concept-agent-install-options.html#agent-cloud-manual-deploy["了解何时在 AWS 中手动部署"]。
...
.. 完成剩余步骤以创建策略。
+
根据您计划使用的 NetApp 数据服务，您可能需要创建第二个策略。对于标准区域，权限分布在两个策略中。由于 AWS 中托管策略的最大字符大小限制，需要两个策略。link:reference-permissions-aws.html["了解有关控制台代理的 IAM 策略的更多信息"]。
...
如果您在创建 Console 代理的同一 AWS 帐户中有 Amazon S3 存储桶，您将看到 Amazon S3 存储系统自动显示在 *Systems* 页面上。 https://docs.netapp.com/us-en/storage-management-s3-storage/index.html["了解如何从NetApp Console管理 S3 存储桶"^]
