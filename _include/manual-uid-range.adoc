...
+
您应保持限制模式处于禁用状态，因为这些步骤描述了如何在标准模式下使用 Console。仅当您拥有安全环境并希望断开此帐户与 Console 后端服务的连接时，才应启用限制模式。如果是这种情况，link:./task-quick-start-restricted-mode.html["按照步骤在受限模式下开始使用NetApp Console"]。
