...
[role="lead"]
您可以在 NetApp Console 中管理您的联合。您可以禁用它，更新过期的凭据，以及在不再需要时将其删除。
...
[NOTE]
====
* 如果您使用NetApp Cloud Central 配置了联合，请通过 *Federation* 页面导入它以在控制台中进行管理。link:task-federation-import.html["了解如何导入您的联盟"]
* 您可以在“审核”页面上查看联盟管理事件，例如启用、禁用和更新联盟。 link:task-monitor-cm-operations.html["了解有关在NetApp Console中监控操作的更多信息。"]
...
. 选择*管理>身份和访问*。
. 选择“*Federation*”选项卡。
. 选择要测试的联盟旁边的操作菜单 image:button_3_vert_dots.png["带有三个垂直点的图标"]，然后选择 *测试连接*。
...
. 选择*管理>身份和访问*。
. 选择“*Federation*”选项卡。
. 选择要禁用的联盟旁边的操作菜单 image:button_3_vert_dots.png["带有三个垂直点的图标"]，然后选择 *禁用*。
...
. 选择要删除的联盟旁边的操作菜单 image:button_3_vert_dots.png["带有三个垂直点的图标"]，然后选择 *删除*。
