...
image:diagram_azure_authentication.png["概念图，显示控制台在进行 API 调用之前，如何从 Microsoft Entra ID 获取身份验证和授权。控制台角色定义了 Active Directory 中的权限。它与一个或多个 Azure 订阅以及代表控制台应用程序的服务主体对象相关联。"]
...
.结果
您可以从"详细信息和凭据"页面切换到其他凭据集 https://docs.netapp.com/us-en/bluexp-cloud-volumes-ontap/task-deploying-otc-azure.html["将系统添加到控制台时"^]。
