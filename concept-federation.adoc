...
NOTE: 当 NetApp Console 处于私有模式时，联盟不可用。link:concept-modes.html["详细了解 NetApp Console 模式"^]。
...
.协议
* link:task-federation-adfs.html["Active Directory 联合身份验证服务 (AD FS)"]
* link:task-federation-saml.html["安全断言标记语言 (SAML) 身份提供者"]


.身份提供者
* link:task-federation-entra-id.html["微软Entra ID"]
* link:task-federation-ping.html["Ping联邦"]
...
[role="quick-margin-para"]
通过创建新应用程序并提供详细信息（如 ACS URL、实体 ID 或其他凭据信息），将身份提供程序配置为信任 NetApp。服务提供商信息因身份提供商而异，请参阅特定身份提供商的文档以了解详细信息。您需要与 IdP 管理员一起完成此步骤。
...
.image:https://raw.githubusercontent.com/NetAppDocs/common/main/media/number-4.png["四个"] 在 Console 中测试您的联合
[role="quick-margin-para"]
在启用联合连接之前对其进行测试。使用控制台中联合页面上的测试选项来验证您的测试用户是否可以成功进行身份验证。如果测试成功，则可以启用连接。

.image:https://raw.githubusercontent.com/NetAppDocs/common/main/media/number-5.png["五"] 在 Console 中启用连接
[role="quick-margin-para"]
