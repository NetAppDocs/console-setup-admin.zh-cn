...
---
= 在 Google Cloud 中手动部署 NetApp Console 代理
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
要在自己的 Linux 主机上手动部署 Console 代理，需要查看主机要求、设置网络、准备 Google Cloud 权限、启用 Google Cloud API、安装 Console，然后提供您准备的权限。

在 Google Cloud 中部署代理会启动运行 Linux 的虚拟机实例，并在您选择的 VPC 中启动 Console 代理软件。

.开始之前
* link:concept-agent-install-options.html#agent-cloud-manual-deploy["了解何时在 Google Cloud 中手动部署"]。
