...
summary: 要从NetApp Console在 Azure 中创建控制台代理，您需要设置网络、准备 Azure 权限，然后创建控制台代理。 
---
= 从 NetApp Console 在 Azure 中部署 Console 代理
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
要在 Azure 中从 NetApp Console 部署 Console 代理，需要设置网络、准备 Azure 权限，然后部署 Console 代理。
...
. 在 *Virtual Machine Authentication* 页面上，选择与设置 Azure 权限方式匹配的身份验证选项：
+
...
. 在“虚拟机身份验证”页面上，选择 Azure 订阅、位置、新资源组或现有资源组，然后为您正在创建的控制台代理虚拟机选择一种身份验证方法。
+
...
. 在 *详细信息* 页面上，输入代理的名称，指定标记，并选择是要 Console 创建具有所需权限的新角色，还是要选择使用 link:reference-permissions-azure.html["所需的权限"] 设置的现有角色。
+
...
如果您在创建控制台代理的同一 Azure 帐户中拥有 Azure Blob 存储，您将看到 Azure Blob 存储自动出现在 *系统* 页面上。 https://docs.netapp.com/us-en/bluexp-blob-storage/index.html["了解如何通过NetApp Console管理 Azure Blob 存储"^]
