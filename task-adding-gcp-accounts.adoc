...
[role="lead"]
在 Google Cloud 中创建控制台代理时，您可以使用相关凭据订阅NetApp Intelligent Services（包括Cloud Volumes ONTAP）和NetApp数据服务。您必须将订阅与您想要使用其凭证来支付服务的代理商关联起来。
...
用新的市场订阅替换当前的市场订阅会更新现有和新的Cloud Volumes ONTAP系统的订阅。
...
通过 Google Cloud Marketplace 订阅NetApp数据服务可能会因权限不正确或错过重定向到控制台而失败。如果发生这种情况，请按照以下步骤完成订阅过程。
