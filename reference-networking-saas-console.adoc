...
| 如果 Console 代理位于 AWS Government 区域：\https://*.blob.core.windows.net \https://cloudmanagerinfraprod.azurecr.io | 在 AWS 政府区域中安装 Console 代理时，获取用于 Console 代理升级的映像。 


| 如果 Console 代理位于 Azure Government 区域：\https://*.blob.core.windows.net \https://occmclientinfragov.azurecr.us | 在 Azure Government 区域中安装 Console 代理时，获取用于 Console 代理升级的映像。 
...
| 当 Console agent 安装在商业区域或主权区域时，获取 Console agent 升级所需的镜像。[cols=3*,options="header,autowidth"] 
|===
